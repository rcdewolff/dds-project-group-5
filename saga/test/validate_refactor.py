...
def check_current_state_persistence_refactor() -> None:
    payment_repo = (ROOT / "payment" / "db_repository.py").read_text(encoding="utf-8", errors="ignore")
    stock_repo = (ROOT / "stock" / "db_repository.py").read_text(encoding="utf-8", errors="ignore")
    payment_app = (ROOT / "payment" / "app.py").read_text(encoding="utf-8", errors="ignore")
    stock_app = (ROOT / "stock" / "app.py").read_text(encoding="utf-8", errors="ignore")

    if "CREATE TABLE IF NOT EXISTS accounts" not in payment_repo:
        raise AssertionError("Payment repository must define an accounts current-state table")
    if "CREATE TABLE IF NOT EXISTS inventory" not in stock_repo:
        raise AssertionError("Stock repository must define an inventory current-state table")

    forbidden_repo_markers = [
    ...
    for marker in forbidden_repo_markers:
        if marker in payment_repo or marker in stock_repo:
            raise AssertionError(f"Legacy log/snapshot persistence marker still present: {marker}")

    forbidden_payment = [
        "get_events_for_user(",
        "insert_user_event(",
        "get_user_snapshot(",
        "update_user_snapshot_versioned(",
    ]
    forbidden_stock = [
        "get_log_events_for_item(",
        "insert_log_event(",
        "get_item_snapshot(",
        "update_snapshot_versioned(",
    ]

    for marker in forbidden_payment:
        if marker in payment_app:
            raise AssertionError(f"Payment app still depends on log/snapshot persistence path: {marker}")
    for marker in forbidden_stock:
        if marker in stock_app:
            raise AssertionError(f"Stock app still depends on log/snapshot persistence path: {marker}")
...
if __name__ == "__main__":
    try:
        ...
        check_current_state_persistence_refactor()
